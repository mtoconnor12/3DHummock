...
for i in range(2):
	for j in range(3):	
		t=np.empty(t2-t1)
		y=np.empty(t2-t1)
		mat = np.loadtxt('run_fromCheckpoint_yr10Debug_EvapFix.' + str(c) + '/surface_outlet_flux.dat')
		print(np.shape(mat))
		a = np.shape(mat)
		for k in range(1,t2-t1):
			if a[0] < 365*(k):
				...
				y[k-t1] = np.sum(mat[365*k-1:365*k,1])
		print(y)
		axarr[i,j].plot(t,np.gradient(y))
		t=np.empty(t2-t1)
		y=np.empty(t2-t1)
		mat = np.loadtxt('run_fromCheckpoint_yr10Debug_EvapFix.' + str(c+1) + '/surface_outlet_flux.dat')
		a = np.shape(mat)
		for k in range(1,t2-t1):
                        if a[0] < 365*(k):
                                t[k-t1] = np.nan
		...
		t=np.empty(t2-t1)
		y=np.empty(t2-t1)
		mat = np.loadtxt('run_fromCheckpoint_yr10Debug_EvapFix.' + str(c+2) + '/surface_outlet_flux.dat')
		a = np.shape(mat)
		for k in range(1,t2-t1):
                        if a[0] < 365*(k):
                                t[k-t1] = np.nan
