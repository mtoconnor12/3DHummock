...
for i in range(1):
	for j in range(np.size(bac)):
		for k in range(np.size(bct)):
			directory = "./run_fromCheckpoint_yr10Debug_EvapFix."+str((j*3) + k + 1)
			print directory
		...
		plt.plot(bct,-wtd[j,:])
plt.legend(['Min Acrotelm','Mean Acrotelm','Max Acrotelm'],loc='lower right')
plt.title('Active Layer Thickness vs. Catotelm Thickness, Grouped by Acrotelm Thickness')
