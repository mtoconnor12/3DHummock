...
runID = 'run_fromCheckpoint_CenturySim'
...
for i in range(2):
	for j in range(3):	
		mat = np.loadtxt('../' + runID + '.' + str(c) + '/subsurface_outlet_flux.dat')
		t = mat[:,0]/86400/365  # time in days
		y = mat[:,1] # thing we're plotting
		axarr[i,j].plot(t,np.log10(y))
		mat = np.loadtxt('../' + runID + '.' +  str(c+1) + '/subsurface_outlet_flux.dat')
		t = mat[:,0]/86400/365  # time in days
		y = mat[:,1] # thing we're plotting
		axarr[i,j].plot(t,np.log10(y))
		mat = np.loadtxt('../' + runID + '.' + str(c+2) + '/subsurface_outlet_flux.dat')
		t = mat[:,0]/86400/365
		y = mat[:,1]
		axarr[i,j].plot(t,np.log10(y))
