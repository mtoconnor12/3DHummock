...
for i in range(len(runPrefixList)):
	...
	for j in range(nruns):
		...
		col_dat = transect_data.transect_data(['saturation_ice'], keys=np.s_[trange], directory=directory)
		...
		plt.hold(True)
	np.savetxt(runPrefixList[i] + '_SaturatedIce.csv',y,delimiter=',')
	print(runPrefixList[i] + '_SaturatedIce.csv' + ' Written!')
	plt.hold(True)
