...
# There are 275 Visdumps: We will save and plot every 30 days within that
t1 = 0
t2 = 275
interval = 1
	...
	for j in range(nruns):
		...
		keys, times, dat = parse_ats.readATS(directory, "visdump_data.h5", timeunits='yr')
                ...
                for k in range(len(trange)):
                        where_unsat = np.where(col_dat[2,k,xPos,:] > 0.1)[-1]
                        #print(where_unsat)
		...
		plt.hold(True)
	np.savetxt(runPrefixList[i] + '_90percentWL_Daily.csv',y,delimiter=',')
	print(runPrefixList[i] + '_90percentWL_Daily.csv' + ' Written!')
	plt.hold(True)
...
plt.savefig(foldername + 'All_90percentWL.eps', format = 'eps', dpi = 1000)
