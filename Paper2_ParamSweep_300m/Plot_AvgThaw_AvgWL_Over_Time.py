...
for i in range(len(runPrefixList)):
	...
	# time for flow is in DAYS, time for thaw is in 30 DAY INCREMENTS.
	#subplotInd = [i/4,i%4]

	axes[i/4,i%4].scatter(trange1,-mean_y,marker='s',color='r')
	#axes[i/4,i%4].set_ylim([-1,0])
	plt.hold(True)
	axes[i/4,i%4].scatter(trange1,-mean_y2,marker='*',color='k')
	axes[i/4,i%4].set_title(runPrefixList[i])
	axes[i/4,i%4].set_xlim([3350,3600])

	axes2 = axes[i/4,i%4].twinx()
	color = 'tab:blue'
	axes2.plot(trange3,mean_y3)
	axes2.set_yscale('log')
	axes2.set_ylim([1E-11,1E-1])
